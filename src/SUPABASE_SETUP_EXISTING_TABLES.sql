-- ============================================
-- SUPABASE TABLE SETUP - USING EXISTING TABLES
-- ============================================
-- This script updates your existing 'users' and 'sizes' tables
-- to work with the CMS system
...
INSERT INTO public.users (username, password, full_name, email, role, is_active)
VALUES 
...
-- Insert default canvas sizes if they don't exist (only if table is empty or has less than 5 rows)
INSERT INTO public.sizes (width, height, price, is_active)
SELECT * FROM (VALUES
  (10, 14, 69.00, true),
  (20, 30, 59.65, true),
  (30, 40, 89.99, true),
...
) AS new_sizes(width, height, price, is_active)
WHERE NOT EXISTS (SELECT 1 FROM public.sizes WHERE public.sizes.width = new_sizes.width AND public.sizes.height = new_sizes.height);
...
SELECT 'Sizes table:' as info;
SELECT COUNT(*) as count FROM public.sizes;

SELECT 'Categories table:' as info;
SELECT COUNT(*) as count FROM public.categories;

SELECT 'Subcategories table:' as info;
SELECT COUNT(*) as count FROM public.subcategories;
