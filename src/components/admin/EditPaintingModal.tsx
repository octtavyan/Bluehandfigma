...
import { useAdmin, CanvasPainting, CategoryData, SubcategoryData } from '../../context/AdminContext';
...
export const EditPaintingModal: React.FC<EditPaintingModalProps> = ({
  ...
  const { updatePainting, addCategory, addSubcategory, sizes, frameTypes: adminFrameTypes } = useAdmin();
  ...
  const initialFormData = {
    ...
    isBestseller: painting.isBestseller || false,
    isActive: painting.isActive !== undefined ? painting.isActive : true,
    orientation: painting.orientation || 'portrait' as 'portrait' | 'landscape' | 'square',
    dominantColor: painting.dominantColor || undefined,
    availableSizes: painting.availableSizes || [], // SIMPLIFIED: Just array of size IDs
    printTypes: painting.printTypes || [] as ('Print Hartie' | 'Print Canvas')[],
  ...
  const [newSize, setNewSize] = useState({ width: '', height: '', price: '' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const selectedSizes = sizes.filter(s => formData.availableSizes.includes(s.id));
    const basePrice = Math.min(...selectedSizes.map(s => s.price));
    ...
    try {
      ...
      alert('A apărut o eroare la salvarea modificărilor. Vă rugăm să încercați din nou.');
    }
  };
    ...
    <div className="fixed inset-0 bg-black/30 flex items-start sm:items-center justify-center z-50 overflow-y-auto" onClick={handleBackdropClick}>
      <div className="bg-white rounded-lg w-full max-w-4xl m-4 my-8 shadow-2xl flex flex-col max-h-[calc(100vh-2rem)]">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 flex-shrink-0">
          <h3 className="text-lg sm:text-xl text-gray-900">Editează Print</h3>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                {sizes.filter(s => s.isActive).map(size => {
                  ...
                  const displayPrice = size.price; // Always use price from sizes table
                  const hasDiscount = size.discount > 0;
                  const discountedPrice = hasDiscount ? displayPrice * (1 - size.discount / 100) : displayPrice;
                        ...
                        {hasDiscount ? (
                          <>
                            <div className="line-through opacity-60">
                              {displayPrice.toFixed(2)} lei
                            </div>
                            <div className="text-red-600 font-medium">
                              {discountedPrice.toFixed(2)} lei (-{size.discount}%)
                            </div>
                          </>
                        ) : (
                          <div>{displayPrice.toFixed(2)} lei</div>
                        )}
                      </div>
                    </button>
                  );
